...
func (service TagService) FindGoodsTags(ctx context.Context, OID dao.PrimaryKey) ([]extends.Tag, error) {
	//SELECT unnest("Tags") as Tag,count("Tags") as Count FROM "Content" where  group by unnest("Tags");
	var tags []extends.Tag
	err := db.GetDB(ctx).Model(model.Goods{}).Select(`unnest("Tags") as "Name",count("Tags") as "Count"`).Where(map[string]interface{}{
		"OID": OID,
	}).Where(`array_length("Tags",1)>0`).Group(`unnest("Tags")`).Order(`"Count" desc`).Find(&tags).Error
	tags = tag.CreateUri(tags)
	return tags, err
}
func (service TagService) FindGoodsByTag(ctx context.Context, OID dao.PrimaryKey, tag extends.Tag, _pageIndex int, orders ...dao.Sort) (pageIndex, pageSize int, total int64, list []*model.Goods, err error) {
	//select * from "Content" where array_length("Tags",1) is null;
	db := db.GetDB(ctx).Model(model.Goods{}).Where(`"OID"=?`, OID).
