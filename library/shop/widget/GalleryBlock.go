...
func (g *GalleryBlock) Render(ctx constrain.IContext) (map[string]any, error) {
	...
	ContentItemList, ContentList := g.galleryBlock(g.Organization.ID, 8)
	outData["ContentItemList"] = ContentItemList
	outData["ContentList"] = ContentList

	return outData, nil
}
func (g *GalleryBlock) galleryBlock(OID dao.PrimaryKey, num int) ([]model.ContentItem, []model.Content) {
