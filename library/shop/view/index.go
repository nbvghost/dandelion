...
func (m *IndexRequest) Render(context constrain.IContext) (r constrain.IViewResult, err error) {
	...
	reply.ShowAtHomeList = repository.ContentItemDao.FindContentItemByShowAtHome(m.Organization.ID)
	reply.StickyTopGoodsTypeList = service.Goods.GoodsType.StickyTopGoodsTypeList(db.Orm(), m.Organization.ID)
