...
func (m *LeaveMessage) HandlePost(context constrain.IContext) (constrain.IResult, error) {
...

	{
		...
		botMessage.WriteString(fmt.Sprintf(">**姓名**:<font color=\"info\">%s</font>\n", leaveMessage.Name))
		botMessage.WriteString(fmt.Sprintf(">**Email**:<font color=\"info\">%s</font>\n", leaveMessage.Email))
		botMessage.WriteString(fmt.Sprintf(">**内容**:<font color=\"info\">%s</font>\n", leaveMessage.Content))
		botMessage.WriteString(fmt.Sprintf(">**IP**:<font color=\"info\">%s</font>\n", leaveMessage.ClientIP))
		for k := range extend {
