...
func (g *ListNew) Handle(ctx constrain.IContext) (constrain.IResult, error) {
	...
	pager := g.ContentService.FindSelectWherePaging(db.Orm(), "ID,Title,Picture,ContentItemID,ContentSubTypeID,Author,Look,FromUrl", "CreatedAt desc", model.Content{}, g.Get.Offset, "ContentItemID=?", g.Get.ContentItemID)
